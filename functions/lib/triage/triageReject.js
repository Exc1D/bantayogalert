...
exports.triageReject = functions.https.onCall(async (data, context) => {
    ...
    await db.runTransaction(async (tx) => {
        ...
        (0, shared_1.validateTransition)(currentState, report_1.WorkflowState.Rejected, reportId);
        const claims = context.auth.token;
        const now = new Date().toISOString();
        const entry = (0, shared_1.buildActivityEntry)('rejected', claims.uid, {
            reason,
            category,
        });
        tx.update(db.collection('reports').doc(reportId), {
            workflowState: report_1.WorkflowState.Rejected,
            updatedAt: now,
        });
        tx.update(db.collection('report_private').doc(reportId), {
            ownerStatus: status_1.WORKFLOW_TO_OWNER_STATUS[report_1.WorkflowState.Rejected],
        ...
        await (0, updateAnalyticsForStateChange_1.updateAnalyticsForStateChange)(tx, db, {
            ...
            nextState: report_1.WorkflowState.Rejected,
            eventAt: now,
        });
        await (0, shared_2.appendAuditEntry)(tx, db, {
            ...
            details: {
                ...
                toState: report_1.WorkflowState.Rejected,
                reason,
