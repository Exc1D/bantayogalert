...
**Domain:** Disaster Reporting / Emergency Coordination Platform
...
```bash
# Core dependencies
npm install react@18.3.28 react-dom@18.3.28
npm install vite@8.0.3 typescript@6.0.2
...
# Firebase
npm install firebase@12.11.0 firebase-functions@7.2.2 firebase-admin@13.7.0

# Mapping
npm install leaflet@1.9.4 react-leaflet@4.2.1 @react-leaflet/core@2.1.1
npm install @types/leaflet
npm install supercluster@8.0.1

# State & Routing
npm install @tanstack/react-query@5.96.2 zustand@5.0.12
npm install react-router@6.5.0 react-router-dom@6.5.0

# Validation & SEO
npm install zod@4.3.6 react-helmet-async@3.0.0

# Image Handling
npm install browser-image-compression@2.0.2
npm install sharp@0.34.5  # Cloud Functions only, NOT in client

# PWA
npm install -D vite-plugin-pwa@1.2.0 workbox-core@7.4.0 workbox-precaching@7.4.0 workbox-routing@7.4.0 workbox-window@7.4.0

# Testing
npm install -D vitest@4.1.2 @testing-library/react@16.3.2 @testing-library/user-event@14.5.2
...
**Problem:** react-leaflet 5.x requires React 19. The project spec uses React 18.

**Solution:** Stick with react-leaflet 4.2.1 which supports React 18.

```
// WRONG — will fail
npm install react@18 react-leaflet@5  // peer dep conflict

// CORRECT
npm install react@18 react-leaflet@4.2.1
```

### 2. Vite 8 + vite-plugin-pwa (MEDIUM SEVERITY)

**Problem:** vite-plugin-pwa 1.2.0 peer deps list Vite up to 7.x, not 8.x.

**Solution:** Test thoroughly after installation. May work with Vite 8 despite peer dep declaration.

### 3. Tailwind CSS v4 Breaking Changes (HIGH SEVERITY)

**Problem:** Tailwind v4 has major breaking changes:
- No more `tailwind.config.js` (CSS-first config)
- New `@tailwindcss/vite` plugin required
- Different theme/breakpoint approach

**Solution:** Use Tailwind CSS 3.4.17. The spec was written for v3 and the project uses the v3 config approach.

### 4. React Router v7 vs v6 (LOW SEVERITY)

**Problem:** React Router v7 renamed packages and has some breaking changes from v6.

**Solution:** Stay on React Router 6.5.0 for stability. The v6 API is well-documented and tested.

## React 18 Strict Mode + Leaflet

React 18 Strict Mode double-mounts components in development. Leaflet MapContainer must be wrapped with a ref guard to prevent double initialization:

```typescript
// src/domains/map/MapRefGuard.tsx
import { useRef, useEffect } from 'react';
import type { Map as LeafletMap } from 'leaflet';

export function useMapRefGuard(mapRef: React.RefObject<LeafletMap | null>) {
  const isInitialized = useRef(false);

  useEffect(() => {
    return () => {
      // Cleanup on unmount only, not on double-mount in StrictMode
      if (isInitialized.current && mapRef.current) {
...
The SPEC already handles this with the sibling-map architecture (MapContainer never unmounts), which is the correct approach.
...
```typescript
// vite.config.ts - Recommended strategies
...
export default defineConfig({
  plugins: [
    VitePWA({
      ...
      includeAssets: ['favicon.ico', 'data/municipalities.geojson'],
      manifest: {
        name: 'Bantayog Alert',
        short_name: 'Bantayog',
        theme_color: '#059669',
      ...
      workbox: {
        ...
        // Firestore/API - network first, fall back to cache
        runtimeCaching: [
          {
...
1. **Service Worker Registration**: Auto-updates but user controls when to refresh
2. **Offline Report Draft**: IndexedDB stores draft; Background Sync submits when online
3. **Cached Feed**: NetworkFirst with 10s timeout; show cached data if offline
4. **Map Tiles**: CacheFirst with 30-day expiration; offline-accessible tiles
5. **Connection Status**: Always-visible indicator when offline
