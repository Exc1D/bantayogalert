...
wave: 1
depends_on: []
requirements_addressed: [FM-01, FM-02, FM-03, FM-04, FM-05, FM-06, FM-07, FM-08]
files_modified:
...
<task type="auto">
  <name>Task 1: Create filterStore.ts — Zustand filter state</name>
  <files>src/stores/filterStore.ts</files>
  <action>
Create `src/stores/filterStore.ts` with:
...
Also add to `src/stores/uiStore.ts`: Add `selectedReportId: string | null` and `setSelectedReportId: (id: string | null) => void` to the UIState interface and implementation. This stores which report is selected for the detail view.
  </action>
  <verify>
grep -r "useFilterStore" src/stores/filterStore.ts && grep -r "setMunicipality" src/stores/filterStore.ts</verify>
  <done>useFilterStore has type, severity, municipalityCode, dateRange state with clearFilters; useUIStore has selectedReportId</done>
</task>
